...
installCA().then(() => {
   ...
   enableProxy();
   const rule = {
     summary: "Detect Xiaohongshu anchor partner start broadcast",
     async beforeSendResponse(req, res) {
       if (req.url.includes("https://robs.xiaohongshu.com/api/sns/live/pre")) {
         const body = JSON.parse(res.response.body);
         const baseUrl = body.data.url.push_url;
...
${content}
         ...
         process.stdin.once("data", () => {
           exec("taskkill /f /t /im Xiaohongshu Live Assistant.exe", () => {});
